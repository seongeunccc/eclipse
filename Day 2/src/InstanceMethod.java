...
public class InstanceMethod {
	...
	public static void main(String[] args) {
		...
		do {
			i = im.input1(sc);// 정수 i값 입력받기
			System.out.print("계산할 부호 입력 : ");
			a = im.input2(sc);// 계산할 부호 입력받기
			j = im.input1(sc);// 정수 j값 입력받기

			r = im.calculate(a, i, j); // 어떤 연산을 적용할지 판단하는 calculate 함수 실행
			im.print(a, i, j, r); // 출력함수 실행

			System.out.println("계산을 계속하시겠습니까?(Y/N)");	//계산을 계속 진행할 지 질문 출력
			b = im.input2(sc); // 대답을 b에 저장하고 Y일 경우 do~while 문 반복 실행
		} while (b == 'Y' || b == 'y');

		System.out.println("고생하셨습니다. 계산을 다시 진행하고 싶으시면 프로그램을 다시 실행해주세요."); //y가 아닐 경우 안내문 출력 후 종료
	}

	public int input1(Scanner sc) { // 숫자를 입력받는 메소드
		System.out.print("계산할 숫자 입력 : ");
		int a = sc.nextInt();
		return a;
	}

	public char input2(Scanner sc) { // 문자를 입력받는 메소드
		char a = sc.next().charAt(0);
		return a;
	}

	public int plus(int i, int j) { // 더하기 메소드
		return i + j;
	}
